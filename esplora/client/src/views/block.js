  ...
  [
    ...
    <div className="container">
      ...
      <div className="stats-table font-p2">
        ...
        { /* SEQUENTIA: checkpoint finality (from getcheckpointinfo) */ }
        { b.finalized != null &&
          <div>
            <div>{t`Finality`}</div>
            <div>{ b.finalized
              ? <span className="seq-finalized">{t`Finalized`}</span>
              : <span className="seq-unfinalized">{t`Not yet finalized`}</span> }</div>
          </div>
        }
        <div>
          <div>{t`Timestamp`}</div>
        ...
        { /* SEQUENTIA: Bitcoin (testnet4) anchor for this block */ }
        { b.bitcoin_anchor &&
          <div>
        ...
        { openBlock == b.id && [
          ...
          : b.ext && b.ext.challenge ? [
            ...
            /* SEQUENTIA: show the parsed PoS BLS committee certificate when
               available, otherwise fall back to the raw solution script. */
            , ...(b.pos_certificate ? posCertRows(b.pos_certificate, t) : [
                <div>
...
// SEQUENTIA: render the decoded PoS BLS committee certificate as detail rows.
const posCertRows = (cert, t) => [
  <div>
    <div>{t`Leader signature`}</div>
    <div className="mono">{cert.leader_sig}</div>
  </div>
, <div>
